...
三个 persona agents 独立回答，不看其他 agent 输出。base model 也独立作答，但只作为 reference，不参与投票。
...
1. 三个 persona 独立回答，不看其他 agent 输出。
2. Base model 作为 synthesizer，读取原题、三个 persona 的独立答案，以及 benchmark 的输出格式要求，然后输出最终答案。
...
2. Round 1：每个 persona 看到另外两个 persona 的 Round 0 答案，然后选择坚持、修正或补充自己的答案。
3. Base model 作为 synthesizer，读取原题、三个 persona 的初始答案、一轮 debate 记录，以及 benchmark 的输出格式要求，然后输出最终答案。
...
- 三个 persona 的展示顺序固定，但需要记录顺序，后续可做 order ablation；
...
| System | 说明 | 目的 |
|---|---|---|
| Base single | Qwen2.5-7B-Instruct 单模型 | 基础线 |
| Base reference agent | base 在每个 benchmark 上独立作答并进入诊断统计 | 判断 persona 相对 base 的分化 |
| Single persona | 每个 adapter 单独跑 | 看 persona-specific strengths |
| Homogeneous MAS | 同一个 adapter 采样 3 次 | 控制更多 samples 的影响 |
| Prompt-persona MAS | 不加载 adapter，只用 prompt 扮演同样 persona | 区分 parameterized persona 和 prompt persona |
| Persona trained MAS | `mathematical + goodness + remorse` | 主方法 |

后续备选比较：

| System | 说明 |
|---|---|
| Oracle MAS | 任一 persona 答对即算答对，用于估计互补上限。 |
...
- 每个 benchmark 10 条；
- base single + 3 个 single persona。
...
- AbstentionBench 100 条；
- DeceptionBench 100 条；
- GPQA Diamond 100 条或可承受的小子集；
...
- 如果 trained-persona MAS 没有低于 homogeneous MAS 的错误相关性，则很难支持 learned heterogeneity。
