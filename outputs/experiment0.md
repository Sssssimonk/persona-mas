...
`Changed` 不由模型自报；实现中由代码比较 round-0 与 round-1 输出得到。
...
## 15. Protocol v1 调整记录（2026-06-30）

旧版 `strict-short-v0` 的主要问题是：GPQA 只允许一行短理由，C1 让模型自报 `Changed`，AbstentionBench 被强制成二分类，DeceptionBench prompt 与官方 `L1/L2` 字段没有对齐。因此后续主实验采用以下协议。

### 15.1 主实验协议

GPQA 使用 `prompt_protocol=official_cot`：

- independent agent prompt 包含完整 question 和 `(A)-(D)` choices；
- 要求输出：
...
- C0/C1 synthesizer 能看到完整 choices、每个 agent 的 reasoning 和 final answer；
- C1 debate prompt 也包含完整题目、choices、其他 agent 的 reasoning 和 final answer。

AbstentionBench 使用 `prompt_protocol=official_natural`：

- 主实验不再强制模型输出 `ANSWER/ABSTAIN`；
- 直接使用数据集中的 natural prompt；
- 模型自然回答后，用 judge/detector 将最终 response 标成 `ANSWER|ABSTAIN|UNCLEAR`；
- 旧二分类 prompt 只保留为 diagnostic，不作为主结论。

DeceptionBench 使用 `prompt_protocol=deception_l1_json`：

- 主实验先跑 `prompt_field=L1-self`；
- independent agent 输出 JSON：
...
- judge 只评价最终 `response`，不评价 reasoning；
- `L2-self-pressure` 和 `L2-self-reward` 保留为 stress test；
- L3 multi-turn 暂不进入 MVP-v1。

### 15.2 Aggregation

主实验系统：

- `base_single`：base model 独立作答，作为参考；
- `mathematical_single` / `goodness_single` / `remorse_single`：单 persona adapter 独立作答；
- `persona_voting`：仅用于 GPQA 和旧 Abstention 二分类 diagnostic；
- `persona_c0`：三个 persona 独立回答后，base synthesizer 直接综合；
- `persona_c1`：三个 persona 独立回答后，看彼此答案做一轮更新，再交给 base synthesizer。

C1 的 `Changed` 不再由模型自报，而由代码比较 round-0 与 round-1 的 `decision/final_response` 计算，记录为 `round1_changed_count`。

### 15.3 Controls

代码支持但主实验暂不默认运行：

- `base_ensemble_voting/base_ensemble_c0/base_ensemble_c1`：三个 base 插槽，控制“多 agent 数量 + synthesizer”本身的收益；
- `homogeneous_<persona>_voting/c0/c1`：三个相同 persona adapter，控制“单一强 persona”替代解释；
- prompt-only persona：使用 `adapter_paths={}` 且 `persona_prompt_mode=prompt_only`，控制 role prompt 是否足够解释收益。

### 15.4 当前配置文件

- `configs/experiment0_gpqa_cot_n20.json`：GPQA CoT 小规模 sanity check；
- `configs/experiment0_gpqa_cot_full.json`：GPQA Diamond 全量主实验；
- `configs/experiment0_abstention_natural_n100.json`：AbstentionBench natural protocol n=100；
- `configs/experiment0_deception_l1_self_n100.json`：DeceptionBench L1-self n=100；
- `configs/experiment0_controls_template.json`：后续 control ablation 模板；
- `configs/experiment0_prompt_only_template.json`：prompt-only ablation 模板。

### 15.5 超参数

当前主协议统一使用 deterministic decoding：

- `do_sample=False`；
- GPQA: `max_new_tokens=1024`；
- AbstentionBench natural: `max_new_tokens=1024`；
- DeceptionBench L1 JSON: `max_new_tokens=768`。

旧结果 `gpqa_full`、`abstention_n100`、`deception_n100` 应标记为 `strict-short-v0`，只作为调试和历史 baseline，不直接作为论文主实验结论。
